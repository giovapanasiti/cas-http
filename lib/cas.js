 ...
 *           e.g. http://callback.example.com:8989/
...
var CAS = module.exports = function CAS(options)
{
  ...
  if (options.external_pgt_url) {
    ...
    if (pgt_url.protocol != 'http:') {
      throw new Error('Option `external_pgt_url` must be http');
    }
    if (!pgt_url.hostname) {
      throw new Error('Option `external_pgt_url` must be a valid url like: http://example.com:8989/callback');
    }
    ...
    if (options.external_proxy_url) {
      ...
      if (!proxy_url.hostname) {
        throw new Error('Option `external_proxy_url` must be a vald url like: http://example.com:8080/');
      }
  ...
  var req = http.get({
    ...
    res.on('end', function() {
      ...
      if (cas_version < 2.0) {
        ...
        callback(new Error('Bad response format. CAS1'));
      }
      ...
      // CAS 2.0 (XML response, and extended attributes)
        ...
        callback(new Error('Bad response format. CAS 2'));
...
CAS.prototype.getProxyGrantingTicket = function(pgtIOU, callback)
{
    ...
    if (this.is_pgt_external) {
        ...
        var req = http.get(urlFetchPGT, function(res) {
            res.on('data', function(chunk) {
                pgt += chunk;
                if (pgt.length > 1e6) {
                  req.connection.destroy();
 ...
 *
 * This is a local http server that listens for incoming connections from
...
CAS.prototype.startPgtServer = function(key, cert, ca, callbackHost, callbackPort, proxyPort)
{
    ...
    // the CAS server.
    var pgtServer = http.createServer(serverOptions);
    ...
    // Deprecated:
    // Proxy server that listens for http connections from other CAS clients
    // and forwards them to the target service. Disabled by default.
    if (proxyPort) {
        var proxyServer = http.createServer(serverOptions);
...
/**
 * Create a CAS proxied HTTP/http request.
...
/**
 * Create a CAS proxied HTTP/http request through an external server.
 *
 * Deprecated.
 *
...
CAS.prototype.proxiedRequestExternal = function(proxyURL, pgtIOU, options, callback)
{
    ...
    var serviceObj;
    if (proxyInfo.protocol.indexOf('http') == 0) {
        serviceObj = http;
    } else {
        serviceObj = http;
    }
